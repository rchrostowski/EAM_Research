...
def filter_series_to_date_range(s, date_range):
    """Filter a time series to the sidebar-selected date range."""
...
def macro_dashboard_figure(data, date_range=None):
    ...
    for i, (name, (_, category, direction)) in enumerate(items):
        ...
        s = filter_series_to_date_range(s_full, date_range)
        ...
        # 52-week context is anchored to the selected range's end date.
        cutoff = s.index[-1] - pd.Timedelta(weeks=52)
        window = s.loc[s.index >= cutoff]
        if not window.empty:
    ...
    title_date = date_range[1] if isinstance(date_range, (tuple, list)) and len(date_range) == 2 else datetime.now().date()
    title_date = pd.Timestamp(title_date).strftime("%B %d, %Y")
    fig.suptitle(
        f"Everstar — FRED Macro Dashboard  |  {title_date}",
...
def macro_convergence_figure(data, date_range=None):
    ...
    for name, (_, category, direction) in FRED_SERIES.items():
        if name not in data or data[name].dropna().empty:
            continue
        selected = filter_series_to_date_range(data[name], date_range)
        if len(selected) < 2:
            continue
        sig = compute_macro_signal(selected, direction)
        records.append({
            "Series": name,
            "Category": category,
    ...
    title_date = pd.Timestamp(title_date).strftime("%B %d, %Y")
    ax.set_title(
        f"Everstar — Signal Convergence  |  {title_date}\n"
...
if page == "Macro Overview":
    ...
    if "macro" not in st.session_state:
        ...
        for name, (sid, category, direction) in FRED_SERIES.items():
            ...
            s = filter_series_to_date_range(data[name], date_range)
            if len(s) < 2:
                continue
            sig = compute_macro_signal(s, direction)
        ...
        if not summary.empty:
            ...
            fig = macro_convergence_figure(data, date_range)
            st.pyplot(fig, clear_figure=True, use_container_width=True)
